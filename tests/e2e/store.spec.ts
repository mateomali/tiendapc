...
test('catalog storefront shows legacy search shell and sellable products', async ({ page }) => {
    ...
    await expect(page.locator('#desktop-catalog-group')).toBeVisible();
...
test('catalog category and subcategory filters follow legacy visibility rules', async ({ page }) => {
    ...
    const categorySelect = page.locator('#desktop-catalog-group');

    await expect(categorySelect).toHaveValue('');
    await expect(page.locator('#desktop-catalog-category')).toHaveCount(0);
    ...
    const subcategorySelect = page.locator('#desktop-catalog-category');
